#ifndef FASTSLAM_PARTICLE_HEADER2
#define FASTSLAM_PARTICLE_HEADER2
...
{
public:
    TestParticle(int num_landmarks): num_lms(num_landmarks) {};
    TestParticle(): num_lms(20), mu_(Eigen::Vector3f::Zero()), sigma_(Eigen::Matrix3f::Identity()) {};
     ...
     */
    void measurementUpdate(const ConeLocation& z, const Eigen::Vector3f& current_pose, const Eigen::Matrix2f& R, const Eigen::Matrix2f& Q, TEST_SLAM_PHASE slam_phase);
     ...
     */
    void updateLandmarkWithCholesky(TestLandmark& lm, const Eigen::Matrix2f& H, const Eigen::Matrix2f& innov_cov, const Eigen::Vector2f& innov_mean);
    ...
    std::vector<TestLandmark> landmarks_;
    std::vector<DataAssociation> known_features, unknown_features;
...
private:
     ...
     * @param innov_cov Covariance innovation from landmark EKF calculation
     */
    void updateLandmark(TestLandmark& lm, const Eigen::Matrix2f& H, const Eigen::Matrix2f& innov_cov, const Eigen::Vector2f& innov_mean);
     ...
     */
    bool sameLandmark(const TestLandmark& lm, const std::vector<TestLandmark>& landmarks_to_check);

    /**
     * 
     */
    float dataAssociations(const Eigen::Matrix2f& lm_innov_cov, const Eigen::Vector2f& lm_innov_mean, const Eigen::Matrix<float, 2, 3>& Hv, const Eigen::Vector2f& zt, const ConeLocation& ob, const TestLandmark& lm, const Eigen::Matrix2f& R);
